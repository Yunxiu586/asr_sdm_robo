...
double declareGetDouble(const std::shared_ptr<rclcpp::Node>& nh, const std::string& name,
                        double default_value) {
  if (!nh->has_parameter(name)) nh->declare_parameter(name, default_value);
  return nh->get_parameter(name).as_double();
}

int declareGetInt(const std::shared_ptr<rclcpp::Node>& nh, const std::string& name,
                  int default_value) {
  if (!nh->has_parameter(name)) nh->declare_parameter(name, default_value);
...
void BsplineOptimizer::setCostFunction(const int& cost_code) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("asr_sdm_trajectory_optimizerimizer"), "cost func: %s", cost_str.c_str());
}
...
void BsplineOptimizer::optimize() {
  ...
  nlopt_opt opt = nlopt_create(static_cast<nlopt_algorithm>(
                                   isQuadratic() ? algorithm1_ : algorithm2_),
                               variable_num_);
  if (!opt) {
    RCLCPP_ERROR(rclcpp::get_logger("asr_sdm_trajectory_optimizerimizer"),
  ...
  try {
    ...
    if (result < 0) {
      RCLCPP_WARN(rclcpp::get_logger("asr_sdm_trajectory_optimizerimizer"),
    ...
    RCLCPP_WARN(rclcpp::get_logger("asr_sdm_trajectory_optimizerimizer"), "[Optimization]: nlopt exception");
    cout << e.what() << endl;
  }
  nlopt_destroy(opt);
  ...
  if (!(cost_function_ & GUIDE))
    RCLCPP_INFO(rclcpp::get_logger("asr_sdm_trajectory_optimizerimizer"), "iter num: %d", iter_num_);
}
